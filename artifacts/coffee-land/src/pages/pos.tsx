...
import { Search, ShoppingBag, Plus, Minus, Trash2, X, Coffee, ChevronRight, Printer, CheckCircle2 } from "lucide-react";
import { 
...
export default function POS() {
  ...
  const addToCart = (item: any) => {
    setCart(prev => {
      ...
      return [...prev, { id: Math.random().toString(36).substr(2, 9), menuItemId: item.id, nameEn: item.nameEn, nameAm: item.nameAm, price: item.price, quantity: 1 }];
    });
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      const result: any = await createOrder.mutateAsync({ data: orderData });
      ...
      toast.success("Order placed — ready to print receipt");
    } catch (err: any) {
  ...
  const handlePrint = () => {
    ...
    printReceipt({
      ...
      tableLabel: completedOrder.tableLabel,
    ...
    <div className="flex h-full bg-muted/20">
      ...
      <div className="w-96 bg-card border-l border-border shadow-2xl flex flex-col z-20 shrink-0 relative">
        ...
        <div className="p-4 border-b border-border bg-muted/30 space-y-4">
          <RadioGroup
            value={orderType}
            onValueChange={(v) => { setOrderType(v as OrderInputOrderType); if (v !== "dine_in") setSelectedTable(null); }}
            className="flex gap-2"
          >
            {[{ id: "dine_in", label: "Dine In" }, { id: "takeaway", label: "Takeaway" }, { id: "delivery", label: "Delivery" }].map(type => (
        ...
        <div className="p-4 border-t border-border bg-card shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          <div className="space-y-2 mb-4">
            <div className="flex justify-between text-muted-foreground">
              <span>Subtotal</span><span>ETB {cartSubtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-muted-foreground">
              <span>Tax (15%)</span><span>ETB {cartTax.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-xl font-bold text-foreground pt-2 border-t border-border">
              <span>Total</span><span className="text-primary">ETB {cartTotal.toFixed(2)}</span>
            </div>
          </div>
          <Button
            size="lg"
      ...
      <Dialog open={showReceipt} onOpenChange={setShowReceipt}>
        <DialogContent className="max-w-sm">
          ...
          {completedOrder && (
            <div className="font-mono text-sm bg-muted/40 rounded-xl p-4 border border-border">
              {/* Header */}
              <div className="text-center mb-3">
                <p className="font-bold text-base">☕ Coffee Land</p>
                <p className="text-xs text-muted-foreground">Order #{completedOrder.orderNumber}</p>
                <p className="text-xs text-muted-foreground capitalize">{completedOrder.orderType.replace("_", " ")}{completedOrder.tableLabel ? ` · ${completedOrder.tableLabel}` : ""}</p>
              </div>
              ...
              <div className="space-y-1 mb-2">
                {completedOrder.items.map(item => (
                  <div key={item.id} className="flex justify-between gap-2">
                    ...
                    <span className="flex-1 truncate">{item.nameEn}</span>
              ...
              <div className="space-y-0.5">
                <div className="flex justify-between text-muted-foreground">
                  <span>Subtotal</span><span>{completedOrder.subtotal.toFixed(2)} ETB</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Tax (15%)</span><span>{completedOrder.tax.toFixed(2)} ETB</span>
                </div>
                <div className="flex justify-between font-bold text-base pt-1">
                  <span>TOTAL</span><span className="text-primary">{completedOrder.total.toFixed(2)} ETB</span>
                </div>
              </div>
              <div className="border-t border-dashed border-border my-2" />
              <p className="text-center text-xs text-muted-foreground">Thank you! አመሰግናለን 🙏</p>
            </div>
          )}
